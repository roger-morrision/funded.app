...
const useRpcProxy = configuredApiBase === '/' || String(import.meta.env.VITE_USE_RPC_PROXY || '').toLowerCase() === 'true';
const rpcProxyUrl = useRpcProxy ? `${globalThis.location?.origin || ''}/api/solana/rpc` : (configuredApiBase ? `${apiBase}/api/solana/rpc` : '');
